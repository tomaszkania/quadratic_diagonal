...
\title[\texttt{quadratic\_diagonal}: exact diagonal representability]{\texttt{quadratic\_diagonal}: Exact constructive diagonal representability over real quadratic orders}

\author{Tomasz Kania}
\authornote{IM CAS (RVO 67985840).}
\affiliation{%
  \institution{Institute of Mathematics, Czech Academy of Sciences, \v{Z}itn\'a 25, 115 67 Prague 1}
  \city{Prague 1}
  \country{Czech Republic}}
\affiliation{%
  \institution{Institute of Mathematics, Jagiellonian University, \L ojasiewicza 6, 30-348 Krak\'ow}
  \city{Krak\'ow}
  \country{Poland}}
\email{tomasz.marcin.kania@gmail.com}
\authorsaddresses{Tomasz Kania, Institute of Mathematics, Czech Academy of Sciences, \v{Z}itn\'a 25, 115 67 Prague 1, Czech Republic; Institute of Mathematics, Jagiellonian University, \L ojasiewicza 6, 30-348 Krak\'ow, Poland; \href{mailto:tomasz.marcin.kania@gmail.com}{tomasz.marcin.kania@gmail.com}}

\begin{document}

\begin{abstract}
\texttt{quadratic\_diagonal} is a typed, dependency-free Python package for exact constructive
representability by diagonal quadratic forms over maximal real quadratic orders. Given a squarefree
radicand, totally positive coefficients, and a totally positive target, the public API decides
representability, returns an integral witness when one exists, and verifies the certificate
independently. The implementation combines exact weighted trace-form enumeration with constructive
dynamic programming, meet-in-the-middle search, and a batched algorithm for all bounded targets. We
prove the weighted discriminant identity, exact row-interval correctness, distinct-value
asymptotics, and pseudo-polynomial arithmetic-step bounds. Every mathematical decision uses integer
trace and norm arithmetic; floating point is confined to reported timings and plotting. The versioned
software component contains command-line drivers, tests, independent validation sweeps, deterministic
reproduction scripts, expected-output summaries, figures, and an executed notebook. A bounded
validation corpus spanning 26 fields produced 2,998 weighted-search comparisons, 2,512 independent
representability comparisons, 2,452 verified constructive certificates, and no discrepancy. The
package is intended both as reusable mathematical software and as an executable specification for
...
in the same spirit as the related public software repository \texttt{quadratic\_sos} \cite{KaniaQSOS},
...
\frac{M_a(\alpha)-1}{2}
\]
canonical non-zero lattice points. Apart from integer-square-root evaluations used to determine row
bounds and endpoints, it performs
\[
O\bigl(M_a(\alpha)+\sqrt{\Tr(\alpha)}+1\bigr)
\]
exact ring and integer operations, up to expected-amortised hash-table costs. It makes
$O(\sqrt{\Tr(\alpha)}+1)$ calls to the exact integer square-root routine. It stores exactly $\#\cS_a(\alpha)$
...
Proposition~\ref{prop:row-interval} computes the exact interval using a constant number of arithmetic operations and exact integer-square-root evaluations. The
...
\begin{remark}[Complexity model]
Unless stated otherwise, the bounds count exact ring operations, integer comparisons, hash-table
operations in expected amortised constant time, and calls to an exact integer-square-root routine.
For fixed $D$ and fixed coefficients, every coordinate encountered by the single-target algorithms
has bit length polynomial in the bit length of the input coordinates. Hence the corresponding bit
cost is obtained by multiplying the arithmetic-step counts by polynomial-time big-integer arithmetic
costs. The pseudo-polynomial statements are deliberately expressed in the numerical parameters
$\Tr(\alpha)$ or $B$; they are not polynomial-time claims in the binary encoding of those bounds.
\end{remark}
...
\begin{remark}[Dependence on the field and rank]
Let $u$ be the number of distinct diagonal coefficients. Combining
Propositions~\ref{prop:single-box-count} and~\ref{prop:caching} gives the more explicit
arithmetic-step bound
\[
O\!\left(
 u\,\Tr(\alpha)
 + r\,\Tr(\alpha)
 \left(\frac{\N(\alpha)}{\sqrt{\Disc(K_D)}}+\Tr(\alpha)+1\right)
\right).
\]
The first term is weighted-set preprocessing with caching, and the second bounds the $r$ dynamic-
programming layers. For a fixed field and fixed lattice this reduces to the statement of
Theorem~\ref{thm:single-pseudopoly}.
\end{remark}
...
\right)
\]
exact arithmetic steps and stores $O(\#L_t(\alpha)+\#R_{r-t}(\alpha))$ distinct half sums. In the constructive
implementation each table entry also stores a root-choice tuple of length at most $r$, so the
witness payload occupies
\[
O\!\left(r\bigl(\#L_t(\alpha)+\#R_{r-t}(\alpha)\bigr)\right)
\]
coefficient-pair slots.
\end{corollary}
...
The implementation constructs each half table with a rolling dictionary: a state stores one complete
choice tuple for the coefficients already processed, and the previous dictionary is discarded when
the next layer has been formed. Thus the final state-table storage is
$O(\#L_t(\alpha)+\#R_{r-t}(\alpha))$. Accounting for the stored constructive tuples multiplies the
number of entries by at most $r$.
\end{proof}
...
\begin{remark}
The implementation includes an exact balancing helper based on first-embedding sign tests. It locates
the unique half-open normal-form representative by exponential search followed by a descending
binary-greedy refinement; an orbit displacement of $n$ unit-square steps therefore needs
$O(\log(1+n))$ exact multiplications and comparisons rather than $O(n)$ one-step iterations. By
...
Version~1.1.0 of the public repository
\url{https://github.com/tomaszkania/quadratic_diagonal} is the software artefact for this paper. It contains the installable package, exact arithmetic in $\OD$,
...
worked examples. The repository is organised in the same spirit as the related public repository
...
\subsection*{Public interface and validation design}

The stable public entry points are summarised below. All algebraic elements are represented by
integer coefficient pairs in the integral basis $\{1,\omega_D\}$.
\begin{table}[tbp]
\centering
\small
\begin{tabularx}{\textwidth}{l X}
\toprule
API entry point & Contract \\
\midrule
\texttt{RealQuadraticOrder} & validates a squarefree radicand and supplies exact order arithmetic and comparisons; \\
\texttt{enumerate\_weighted\_search} & returns distinct values $ax^2\preccurlyeq\alpha$, one root certificate per value, and structural search statistics; \\
\texttt{diagonal\_representability\_dp} & decides representability and returns a verified root tuple by constructive dynamic programming; \\
\texttt{diagonal\_representability\_mitm} & provides the same contract through a constructive meet-in-the-middle solver; \\
\texttt{batched\_bounded\_representables} & computes all represented targets below a trace bound in one pass; \\
\texttt{verify\_representation} & independently checks that a returned root tuple has the requested coefficientwise sum. \\
\bottomrule
\end{tabularx}
\caption{Principal public API of \texttt{quadratic\_diagonal} 1.1.0.}
\label{tab:api}
\end{table}

The validation path is intentionally more redundant than the production path. Exact row search is
compared with an independent sharp-box scan; DP and meet-in-the-middle are compared with explicit
Cartesian products on bounded instances; every constructive output is checked by
\texttt{verify\_representation}; and batched bounded search is compared with independent per-target
recomputation. In the full bundled sweep over all squarefree $D\le 41$ and the stated bounded
coefficient/target ranges, this gives the deterministic totals in Table~\ref{tab:validation-summary}.
\begin{table}[tbp]
\centering
\small
\begin{tabular}{rrrrr}
\toprule
Fields & weighted searches & representability cases & certificates & bounded cases \\
\midrule
26 & 2998 & 2512 & 2452 & 104 \\
\bottomrule
\end{tabular}
\caption{Independent validation corpus; every comparison agreed.}
...
\bottomrule
\end{tabular}
\caption{Internal Cartesian-product exact baseline for single-target representability. The ``generic'' column expands the full Cartesian product of the value lists after the same exact preprocessing used by DP and meet-in-the-middle.}
\label{tab:generic}
\end{table}

Table~\ref{tab:generic} isolates the combination-layer advantage of the specialised solvers. The artefact does not depend on an
external CAS, so we bundle a reproducible internal Cartesian-product baseline that mirrors the post-enumeration task a general-purpose system would still have to
solve: given the exact weighted-value sets, search their full Cartesian product for a representation.
This is not presented as a proxy for the complete behaviour of any particular computer algebra system; it keeps the comparison portable while showing the algorithmic advantage of reusing partial
...
Table~\ref{tab:arith} gives illustrative arithmetic output: explicit bounded truant
...
The complete source, tests, examples, notebook, deterministic data, and reproduction drivers for
version~1.1.0 are available at
\url{https://github.com/tomaszkania/quadratic_diagonal}. The TOMS software component is generated
from that source by \path{scripts/make_submission_archive.py}; it includes SHA-256 checksums and
requires no proprietary dependency. No external research data were used.
